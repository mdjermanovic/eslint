...
class Linter {
    ...
    async _verifyWithFlatConfigArrayAndProcessor(textOrSourceCode, config, options, configForRecursive) {
        ...
        const blocks = preprocess(text, filenameToExpose).map((block, index) => [block, index]);
        const messageLists = [];
        ...
        for (const [block, index] of blocks) {
            ...
            if (typeof block === "string") {
                messageLists.push(await this._verifyWithFlatConfigArrayAndWithoutProcessors(block, config, options));
                continue;
            }

            const blockText = block.text;
            const blockName = path.join(filename, `${index}_${block.filename}`);
            ...
            if (configForRecursive && (text !== blockText || path.extname(blockName) !== originalExtname)) {
                ...
                messageLists.push(await this._verifyWithFlatConfigArray(
                    blockText,
                    configForRecursive,
            ...
            // Does lint.
            messageLists.push(await this._verifyWithFlatConfigArrayAndWithoutProcessors(
                blockText,
                config,
                { ...options, filename: blockName, physicalFilename }
            ));
    ...
    async _verifyWithProcessor(textOrSourceCode, config, options, configForRecursive) {
        ...
        for (const [block, index] of blocks) {
            ...
            // Keep the legacy behavior.
            if (typeof block === "string") {
                messageLists.push(await this._verifyWithoutProcessors(block, config, options));
                continue;
            }
            ...
            if (configForRecursive && (text !== blockText || path.extname(blockName) !== originalExtname)) {
                debug("Resolving configuration again because the file content or extension was changed.");
                messageLists.push(await this._verifyWithConfigArray(
                    blockText,
                    configForRecursive,
                    { ...options, filename: blockName, physicalFilename }
                ));
            ...
            // Does lint.
            messageLists.push(await this._verifyWithoutProcessors(
                blockText,
                config,
                { ...options, filename: blockName, physicalFilename }
            ));
     ...
     * @returns {Promise<{fixed:boolean,messages:LintMessage[],output:string}>} The result of the fix operation as returned from the
     *      SourceCodeFixer.
     */
