...
function convertToFlat(config) {
    ...
    if (config.env) {
        for (const [key, value] of Object.entries(config.env)) {
            ...
            switch (key) {
                case "es6":
                case "es2015":
                    flatConfig.languageOptions.ecmaVersion = 2015;
                    break;

                case "es2017":
                    flatConfig.languageOptions.ecmaVersion = 2017;
                    break;

                case "es2020":
                    flatConfig.languageOptions.ecmaVersion = 2020;
                    break;

                case "es2021":
                    flatConfig.languageOptions.ecmaVersion = 2021;
                    break;

                case "node":
                    flatConfig.languageOptions.sourceType = "commonjs";
                ...
                case "browser":
                    ...
                    delete flatConfig.languageOptions.globals["AudioWorkletGlobalScope "]; // bug in globals library?
                    break;
    ...
    if (config.parserOptions) {
        ...
        if (config.parserOptions.ecmaVersion) {
            flatConfig.languageOptions.ecmaVersion = config.parserOptions.ecmaVersion;
            if (config.parserOptions.ecmaVersion >= 2015) {
                flatConfig.languageOptions.parserOptions.ecmaVersion -= 2009; // old normalization in parserOptions
            }
        }

        if (config.parserOptions.sourceType) {
            flatConfig.languageOptions.sourceType = config.parserOptions.sourceType;
        }
    }

    if (config.parser) {
        flatConfig.languageOptions.parser = require(config.parser);
    }
...
class FlatRuleTester {
    ...
    constructor(testerConfig = {}) {
        ...
        this.testerConfig = [
            sharedDefaultConfig,
            { languageOptions: { ecmaVersion: 5, sourceType: "script" } }, // old defaults
            testerConfig.plugins ? testerConfig : convertToFlat(testerConfig),
    ...
    run(ruleName, rule, test) {
        ...
        function runRuleForItem(item) {
            ...
            if (typeof item === "string") {
                 ...
                 */
                const itemConfig = convertToFlat(item);
